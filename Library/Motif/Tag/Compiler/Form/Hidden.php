...
/**
 * Compile <motif:form:hidden var="dealTitle" />
 */
class Motif_Tag_Compiler_Form_Hidden extends Motif_Tag_Compiler_Abstract
{

    /**
     * @var string Tag's name
     */
    protected $_tagName = 'form:hidden';
            ...
            $this->_replaceCode(sprintf(
                '<motif:choose>' . NL .
                    '<motif:when var="%1$s">' . NL .
                        '<input type="hidden" name="%1$s" id="%1$s" value="{%1$s}"%3$s />' . NL .
                    '</motif:when>' . NL .
                    '<motif:otherwise>' .
                        '<input type="hidden" name="%1$s" id="%1$s" value="%2$s"%3$s />' . NL .
                    '</motif:otherwise>' . NL .
                '</motif:choose>',
                $inputName,
                $inputValue,
